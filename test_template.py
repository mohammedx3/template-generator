...
try:
    with open(TEMPLATE_FILE_LOCATION, "r", encoding="UTF-8") as template_file:
...
try:
    MAIN_STACK = CF_RESOURCE.Stack(STACK_NAME)
    OUTPUT_VARIABLES = outputs(MAIN_STACK)

    NAT_GATEWAY_ID = OUTPUT_VARIABLES["nat_gateway_id"]
    NAT_EIP = OUTPUT_VARIABLES["nat_eip"]
    INTERNET_GATEWAY_ID = OUTPUT_VARIABLES["internet_gateway_id"]
    PRIVATE_SUBNET_ID = OUTPUT_VARIABLES["private_subnet_id"]
    PUBLIC_SUBNET_ID = OUTPUT_VARIABLES["public_subnet_id"]
    PROTECTED_SUBNET_ID = OUTPUT_VARIABLES["protected_subnet_id"]
    PUBLIC_ROUTE_TABLE_ID = OUTPUT_VARIABLES["public_route_table_id"]
    PROTECTED_ROUTE_TABLE_ID = OUTPUT_VARIABLES["protected_route_table_id"]
    VPC_ID = OUTPUT_VARIABLES["vpc_id"]
except:
...
def test_protected_route_table():
    ...
    assert (
        first_route["DestinationCidrBlock"] == VPC_CIDR
        and first_route["GatewayId"] == "local"
    )
    assert (
        second_route["DestinationCidrBlock"] == "0.0.0.0/0"
        and second_route["NatGatewayId"] == NAT_GATEWAY_ID
    )
...
def test_public_route_table():
    ...
    assert (
        second_route["DestinationCidrBlock"] == "0.0.0.0/0"
        and second_route["GatewayId"] == INTERNET_GATEWAY_ID
    )
